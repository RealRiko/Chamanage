...
@section('title', __('page.products.title') . ' — ' . config('app.name', 'Chamanage'))
...
<div class="pg-shell products-page">
    <div style="max-width: 1200px; margin: 0 auto; padding: 40px 24px 64px;">
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 20px; align-items: flex-end; justify-content: space-between;">
                <div>
                    <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 12px; flex-wrap: wrap;">
                        <span class="pg-eyebrow">{{ __('page.products.badge') }}</span>
                        @if ($hasCatalog)
                            <span class="count-pill">
                                ...
                                {{ $catalogTotal }} {{ __('page.products.stat_items') }}
                            </span>
                        @endif
                    </div>
                    <h1 class="pg-title">{{ __('page.products.title') }}</h1>
                    <p class="pg-subtitle" style="margin-top: 8px; max-width: 480px;">{{ __('page.products.subtitle') }}</p>
                </div>
                <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                    <a href="{{ route('inventory.index') }}" class="btn-ghost">
        ...
        <div class="main-card">
            ...
            @if ($showToolbar)
                <div class="toolbar">
                    <form method="GET" action="{{ route('products.index') }}" style="display: flex; flex-wrap: wrap; gap: 8px; align-items: center;">
                        <div class="search-wrap">
                            ...
                            <input
                                ...
                                autocomplete="off"
                                class="search-input"
                                placeholder="{{ __('page.products.search_placeholder') }}"
                            >
                        </div>
                        <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                            <button type="submit" class="btn-search">{{ __('page.products.filter_submit') }}</button>
                            @if ($search !== '')
                                <a href="{{ route('products.index') }}" class="btn-clear">{{ __('page.products.filter_clear') }}</a>
                ...
                @if ($products->isEmpty())
                    ...
                    <div class="empty-state">
                        ...
                        @if ($search !== '')
                            ...
                            <a href="{{ route('products.index') }}" class="btn-primary" style="margin: 0 auto;">{{ __('page.products.filter_clear') }}</a>
                        @else
                            <h3 class="empty-title">{{ __('page.products.empty_title') }}</h3>
                            <p class="empty-hint">{{ __('page.products.empty_hint') }}</p>
                            <a href="{{ route('products.create') }}" class="btn-primary" style="margin: 0 auto;">
                ...
                @else
                    ...
                    >
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; flex-wrap: wrap; gap: 12px;">
                            <p style="font-size: 13px; color: #a8a29e; margin: 0;">{{ __('page.products.view_label') }}</p>
                        ...
                        <div x-show="view === 'grid'" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0">
                            <ul id="products-grid-ul" class="products-grid" style="list-style: none; margin: 0; padding: 0;">
                                @foreach ($products as $product)
                                    @php $initial = mb_strtoupper(mb_substr($product->name ?: '?', 0, 1, 'UTF-8')); @endphp
                                    <li>
                                        <article class="product-card">
                                            <div class="card-header">
                                                ...
                                                <div style="flex: 1; min-width: 0;">
                                                    ...
                                                    <p class="card-id">ID {{ $product->id }}</p>
                                                </div>
                                            ...
                                            <div class="card-body">
                                                ...
                                                @endif
                                                <p class="card-desc">{{ $product->description ? \Illuminate\Support\Str::limit($product->description, 100) : '—' }}</p>
                                            </div>
                                            <div class="card-footer">
                                                <a href="{{ route('products.edit', $product) }}" class="icon-btn" title="{{ __('page.action_edit') }}">
                                                ...
                                                <button type="button" class="icon-btn danger product-delete-trigger" title="{{ __('page.action_delete') }}" data-product-id="{{ $product->id }}" data-product-name="{{ e($product->name) }}">
                                ...
                                <table class="data-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="hidden lg:table-cell">{{ __('page.products.col_desc') }}</th>
                                    ...
                                    <tbody id="products-table-tbody">
                                        @foreach ($products as $product)
                                            ...
                                            <tr>
                                                ...
                                                <td class="hidden lg:table-cell" style="max-width: 260px; color: #78716c; font-size: 13px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{ $product->description ? \Illuminate\Support\Str::limit($product->description, 60) : '—' }}</td>
                                                <td>
                                                    <div style="display: flex; justify-content: flex-end; gap: 4px;">
                                                        <a href="{{ route('products.edit', $product) }}" class="icon-btn">
                                                            <svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                                        </a>
                                                        <button type="button" class="icon-btn danger product-delete-trigger" data-product-id="{{ $product->id }}" data-product-name="{{ e($product->name) }}">
                                                            <svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                                        </button>
                                                    </div>
                                                </td>
...
<div id="deleteModal" class="modal-overlay" role="dialog" aria-modal="true" aria-labelledby="delete-modal-title">
    <div class="modal-box">
        <div class="modal-body">
            ...
            <p class="modal-desc">{{ __('page.products.delete_confirm') }} <strong id="productName" class="modal-entity-name"></strong>?</p>
        </div>
...
const productSearchNone = @json(__('page.products.empty_search_title'));
