...
<head>
    ...
    <title>{{ __('auth.login_title') }} — {{ config('app.name', 'Chamanage') }}</title>
    ...
    <header class="relative z-20 border-b border-zinc-200/80 dark:border-white/10">
        <div class="mx-auto flex max-w-lg items-center justify-between gap-4 px-4 py-5 sm:px-6">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('register') }}" class="text-sm font-medium text-zinc-700 transition hover:text-zinc-900 dark:text-white/80 dark:hover:text-white">{{ __('auth.sign_up_link') }}</a>
            </div>
        </div>
    </header>
            ...
            <div class="glass rounded-3xl p-8 shadow-2xl sm:p-10">
                <div class="mb-8 text-center">
                    <h1 class="text-3xl font-extrabold tracking-tight sm:text-4xl">
                        <span class="bg-gradient-to-r from-amber-200 via-brand to-amber-300 bg-clip-text text-transparent">{{ __('auth.welcome_back') }}</span>
                    </h1>
                    <p class="mt-2 text-sm text-zinc-600 dark:text-white/55">{{ __('auth.subtitle') }}</p>
                </div>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input id="password" type="password" name="password" required autocomplete="current-password"
                            class="w-full rounded-xl border border-zinc-300/80 bg-white/85 px-4 py-3.5 text-zinc-900 shadow-inner placeholder:text-zinc-400 transition focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/40 dark:border-white/20 dark:bg-white/10 dark:text-white dark:placeholder:text-white/35 @error('password') border-red-400/70 ring-2 ring-red-500/40 @enderror"
                            placeholder="••••••••" />
                        @error('password')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-300">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3 pt-1">
                        <label class="inline-flex cursor-pointer items-center gap-2">
                            ...
                            <span class="text-sm text-zinc-600 dark:text-white/65">{{ __('auth.remember') }}</span>
                        </label>
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-sm font-medium text-amber-300/90 transition hover:text-amber-200">{{ __('auth.forgot') }}</a>
                        @endif
                    </div>
                ...
                <p class="mt-8 text-center text-sm text-zinc-600 dark:text-white/50">
                    {{ __('auth.new_here') }}
                    <a href="{{ route('register') }}" class="font-semibold text-amber-300 transition hover:text-amber-200">{{ __('auth.create_account') }}</a>
                </p>
            </div>
