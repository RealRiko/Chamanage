...
<head>
    ...
    <title>{{ __('auth.register_title') }} — {{ config('app.name', 'Chamanage') }}</title>
...
<body class="auth-page min-h-screen text-zinc-900 antialiased transition-colors dark:text-white">
    ...
    <header class="relative z-20 border-b border-zinc-200/80 dark:border-white/10">
        <div class="mx-auto flex max-w-2xl items-center justify-between gap-4 px-4 py-5 sm:px-6">
            <a href="{{ route('home') }}" class="flex items-center gap-2 text-base font-bold tracking-tight">
                ...
                <span>{{ config('app.name', 'Chamanage') }}</span>
            </a>
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('login') }}" class="text-sm font-medium text-zinc-700 transition hover:text-zinc-900 dark:text-white/80 dark:hover:text-white">{{ __('nav.login') }}</a>
            </div>
        </div>
    </header>
            ...
            <div class="glass rounded-3xl p-8 shadow-2xl sm:p-10">
                <div class="mb-8 text-center">
                    <h1 class="text-3xl font-extrabold tracking-tight sm:text-4xl">
                        <span class="bg-gradient-to-r from-amber-200 via-brand to-amber-300 bg-clip-text text-transparent">{{ __('auth.register_hero') }}</span>
                    </h1>
                    <p class="mt-2 text-sm text-zinc-600 dark:text-white/55">{{ __('auth.register_sub') }}</p>
                </div>
                ...
                <form class="space-y-5" method="POST" action="{{ route('register.store') }}">
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <label for="name" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-white/70">{{ __('auth.name') }}</label>
                        ...
                        <div>
                            <label for="surname" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-white/70">{{ __('auth.surname') }}</label>
                    ...
                    <div>
                        <label for="email" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-white/70">{{ __('auth.email') }}</label>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <label for="password" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-white/70">{{ __('auth.password') }}</label>
                            <input id="password" type="password" name="password" required autocomplete="new-password"
                                class="w-full rounded-xl border border-zinc-300/80 bg-white/85 px-4 py-3.5 text-zinc-900 shadow-inner placeholder:text-zinc-400 transition focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/40 dark:border-white/20 dark:bg-white/10 dark:text-white dark:placeholder:text-white/35">
                        </div>
                        <div>
                            <label for="password_confirmation" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-white/70">{{ __('auth.confirm_password') }}</label>
                    ...
                    <div>
                        <label for="country" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-white/70">{{ __('auth.country') }}</label>
                        <select id="country" name="country" required
                            ...
                            <option value="" disabled @selected(! old('country')) class="text-zinc-400 dark:text-white/50">{{ __('auth.select_country') }}</option>
                            @foreach (['LV', 'LT', 'EE'] as $code)
                                <option value="{{ $code }}" @selected(old('country') === $code)>{{ __('country.'.$code) }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div>
                        <label for="company_name" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-white/70">{{ __('auth.company_name') }}</label>
                        <input id="company_name" type="text" name="company_name" value="{{ old('company_name') }}" required placeholder="SIA Example Co."
                            class="w-full rounded-xl border border-zinc-300/80 bg-white/85 px-4 py-3.5 text-zinc-900 shadow-inner placeholder:text-zinc-400 transition focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/40 dark:border-white/20 dark:bg-white/10 dark:text-white dark:placeholder:text-white/35">
                    </div>
                ...
                <p class="mt-8 text-center text-sm text-zinc-600 dark:text-white/50">
                    ...
                    <a href="{{ route('login') }}" class="font-semibold text-amber-300 transition hover:text-amber-200">{{ __('auth.sign_in_link') }}</a>
                </p>
            </div>

            <p class="mt-8 text-center text-xs text-zinc-500 dark:text-white/35">
                <a href="{{ route('home') }}" class="transition hover:text-zinc-700 dark:hover:text-white/55">{{ __('auth.back_home') }}</a>
            </p>
        </div>
    </main>
    <script>
        (() => {
            ...
            const darkTitle = @js(__('theme.dark'));
            const lightTitle = @js(__('theme.light'));
