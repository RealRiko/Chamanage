...
<form id="documentForm" action="<?php echo e(isset($document) ? route('documents.update', $document) : route('documents.store')); ?>" method="POST" class="space-y-6">
    <?php echo csrf_field(); ?>
    <?php if(isset($document)): ?>
        <?php echo method_field('PATCH'); ?>
    <?php endif; ?>

    <?php echo $__env->make('documents._document_fields', ['document' => $formDocument], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    ...
    <div class="flex flex-col-reverse gap-3 border-t border-zinc-200/80 pt-8 sm:flex-row sm:items-center sm:justify-between dark:border-white/10">
        <a href="<?php echo e(route('documents.index')); ?>" class="btn-secondary justify-center sm:justify-start">
            ← <?php echo e(__('form.back')); ?>

        </a>
        <button type="submit" id="submitBtn" class="btn-primary px-8 py-3">
            <?php echo e(isset($document) ? __('form.document.submit_update') : __('form.document.submit_create')); ?>
...
document.addEventListener('DOMContentLoaded', () => {
    const VAT_RATE = <?php echo json_encode($vatRate, 15, 512) ?>;
    const VAT_MULTIPLIER = <?php echo json_encode($vatMultiplier, 15, 512) ?>;
    const STATUS_LABELS = <?php echo json_encode($docStatusLabels, 15, 512) ?>;
    const MSG_OUT_OF_STOCK = <?php echo json_encode($docMsgOutOfStock, 15, 512) ?>;
    const MSG_QTY_EXCEEDS = <?php echo json_encode($docMsgQtyExceeds, 15, 512) ?>;

    // --- Status Dropdown ---
    const typeSelect = document.getElementById('type');
    const statusSelect = document.getElementById('status');
    const currentStatus = <?php echo json_encode(old('status', $formDocument?->status ?? ''), 512) ?>;
    ...
    let lineIndex = <?php echo e(count($lineItems)); ?>;
    ...
    function updateSummary() {
        ...
        profitLinesBody.innerHTML = rows.length
            ? rows.join('')
            : `<tr><td colspan="4" class="doc-summary-empty">${<?php echo json_encode(__('form.document.summary_no_lines'), 15, 512) ?>}</td></tr>`;
    }
...
<?php $__env->stopPush(); ?>
<?php /**PATH /home5/deveralv/chamanage.lat/CRM-main/resources/views/documents/_document_form.blade.php ENDPATH**/ ?>